...
fs::path scratch(const std::string &name)
{
    ...
    base /= "seabass-trailer-hook-test-" + name;
    std::error_code ec;
...
void testCommitMsgStripsTrailers()
{
    ...
    assert(run((HooksDir / "commit-msg").string() + " " + message.string() + " 2>/dev/null") == 0);
...
struct Repo
{
    ...
    {
        return "git -C " + dir.string() + " -c user.name=Test -c user.email=test@example.org " + args;
    }

    std::string revParse(const std::string &rev) const
    {
        const fs::path out = dir / ".rev";
        [[maybe_unused]] const int status = run(git("rev-parse " + rev) + " > " + out.string());
    ...
    void commit(const std::string &message) const
    {
        ...
        assert(run(git("commit -q --no-verify -F " + msgFile.string())) == 0);
    }
};
...
Repo makeRepo(const std::string &name)
{
    ...
    assert(run("git init -q -b master " + repo.dir.string()) == 0);
    assert(run("git init -q --bare " + repo.remote.string()) == 0);
    assert(run(repo.git("remote add origin " + repo.remote.string())) == 0);
...
int runPrePush(const Repo &repo, const std::string &localSha, const std::string &remoteSha)
{
    ...
    return run("cd " + repo.dir.string() + " && printf '%s\\n' '" + line + "' | " + (HooksDir / "pre-push").string()
               + " origin " + repo.remote.string() + " >/dev/null 2>&1");
}
