...
// Library Health's cover-art notice, both ways round: away on a library
// with nothing to report, and up with the right words on one where every
// track's art points at the computer that ran Engine's import.
//
...
TestCase {
    ...
    function test_theCoverArtNoticeDoesNotOfferAReimportForDeletedImages() {
        ...
        var notice = findChild(page, "artworkNotice");
        compare(notice.visible, true);
        var explanation = findChild(page, "artworkNoticeExplanation");
    ...
    function test_theCoverArtNoticeStaysAwayWithNothingToReport() {
        ...
        tryVerify(function() { return controller.busy === false; }, 60000, "the scan must finish");
        var notice = findChild(page, "artworkNotice");
        verify(notice !== null, "the notice must exist, hidden");
        compare(notice.visible, false, "nothing to report, so nothing shown");
    ...
    function test_theCoverArtNoticeNamesTheFaultItFound() {
        ...
        var notice = findChild(page, "artworkNotice");
        verify(notice !== null);
        compare(notice.visible, true, "a library in this state must say so");
        var headline = findChild(page, "artworkNoticeHeadline");
        ...
        var button = findChild(page, "repairArtworkButton");
        verify(button !== null);
        compare(button.enabled, false, "nothing to copy in, so nothing to offer");
    }
}
