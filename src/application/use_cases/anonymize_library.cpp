      ...
      + (summary.rekordboxPlaceholdersTruncated > 0
             ? "  CUT SHORT: " + std::to_string(summary.rekordboxPlaceholdersTruncated)
                 + " field(s) had no room for a whole\n"
                   ...
                   "    cut to fit. Track titles, filenames and paths put their\n"
                   "    hash first, so what a cut takes from those is the\n"
                   "    readable tail. The numbered ones (artists, albums,\n"
                   "    genres, labels, playlists, cue comments) put the number\n"
                   "    last, so a short field can lose the digits and leave two\n"
                   "    rows reading alike. That is this format, not the scrub\n"
                   "    going wrong, and it is worth knowing before you read two\n"
                   "    identical names as a bug.\n"
             : std::string())
